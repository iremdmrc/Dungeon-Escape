...
void ADungeonEscapeCharacter::Interact()
{
	...
	DrawDebugLine(GetWorld(), Start, End, FColor::Purple, false, 5.0f);
	...
	if (HasHit) {
		...
		UE_LOG(LogTemp, Warning, TEXT("Interact hit: %s | Tags: %s | HasCollectableItemTag: %d"),
			*GetNameSafe(HitActor),
			*FString::JoinBy(HitActor->Tags, TEXT(", "), [](const FName& Tag) { return Tag.ToString(); }),
			HitActor->ActorHasTag("CollectableItem"));
		...
		else if (HitActor->ActorHasTag("Lock"))
		{
			...
			if (LockActor)
			{
				...
				// 3- remove the item from our itemlist
				// 4- activate the lock
			}
		}
	}
